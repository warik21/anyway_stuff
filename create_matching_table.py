...
def create_matching_row(waze_row, redash_row):
    ...
    return [int(same_day), int(same_city), int(same_road), int(same_street), int(same_location), int(time_diff)]


if __name__ == '__main__':
    entries = ["waze uuid", "redash id", "day", "city", "road", "street", "location", "time"]
    ...
    for i in range(len(ground_truth_df.index)):
        row = generator.__next__()
        waze_curr = waze_df.loc[waze_df['uuid'] == row[1][1]]
        redash_curr = redash_df.loc[redash_df['id'] == row[1][0]]
        matching_params_row = create_matching_row(waze_curr, redash_curr)
        df_row = [row[1][1], row[1][0]]
    ...
    print(df.head())
    df.to_excel('matching_with_params.xlsx', sheet_name='Sheet1')
